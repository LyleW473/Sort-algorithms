...
def BubbleSort(list, height_scale):
    ...
    while sort_index < len(list) - 1 : 
        ...
        # Add the list of scaled items to another list which will hold all stages of the sort as well as a tuple containing the two elements being checked in that stage
        sort_stages_list.append((temp_list, (sort_index, sort_index + 1)))
...
while run:
    ...
    if sorting_process == True:
        ...
        for i in range(0, num_of_list_items):
            # Check if i is equal to the values inside the stage tuple for the indexes which indicate which elements are being swapped
            if i == sort_stages_list[stage_index][1][0] or i == sort_stages_list[stage_index][1][1]:
                # If it is, those are the current elements being swapped, so change their colour and line thickness
                rect_colour = BLUE
            # Otherwise the default colour for all elements should be white
            else: 
                rect_colour = WHITE

            pygame.draw.rect(screen, rect_colour, ((border_x_position - 5) + (i * item_width), 695 - (sort_stages_list[stage_index][0][item_index + i]), item_width, sort_stages_list[stage_index][0][item_index + i]), 5)
            # Draw a text at the in the centre of each item, at the y co-ordinate, 500
            draw_text(str(round(sort_stages_list[stage_index][0][item_index + i] / maximum_scale)), item_font, BLACK, (border_x_position - 5) + (i * item_width) + (item_width / 2) - 14, 500)
    ...
    if sorting_process == False and input_process == False:
        ...
        if pygame.time.get_ticks() - last_swap_time < 5000:
            ...
            for i in range(0, len(user_items_list)):
                pygame.draw.rect(screen, GREEN, ((border_x_position - 5) + (i * item_width), 695 - (sort_stages_list[stage_index][0][item_index + i]), item_width, sort_stages_list[stage_index][0][item_index + i]), 5)
                draw_text(str(round(sort_stages_list[stage_index][0][item_index + i] / maximum_scale)), item_font, BLACK, (border_x_position - 5) + (i * item_width) + (item_width / 2) - 14, 500)
